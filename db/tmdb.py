...
def get_film():
	...
	for f in film:
		import datetime
		import time
		print(str(datetime.datetime.now())+" : "+str(f["id"])+"\n")
		# === in film
		info=tmdb.Movies(f["id"]).info(language="it")
		info["voto"]=None
		if x["release_date"]=="": x["release_date"]=None
		...
		if f["collezione"]!=None:
			...
			if (f["collezione"] not in [c["id"] for c in collezione]):
				...
				film.extend([{"id": i["id"]} for i in c["parts"] if i["id"] not in [k["id"] for k in film]])
		...
		if credits["cast"]!=None:
			for x in credits["cast"]:
				...
				persona.extend([{keys[i][0]: p[keys[i][1]] for i in range(len(keys))}])
...
def db_insert(var, nome, engine):
	pd.DataFrame(var).to_csv("dump_csv/"+nome+".csv", index=False)
...
def db_insert_all():
	...
	db_del(film_persona, "film_persona", engine)
	...
	db_insert(film_persona, "film_persona", engine)
